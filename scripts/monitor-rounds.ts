...
const BETTING_WINDOW = 5; // 5 seconds for betting
const COOLDOWN_PERIOD = 3; // 3 seconds cooldown
...
async function getRoundData(client: AptosClient, roundId: any) {
    try {
        ...
        console.log("Debug - Round Data:", roundData);
        return roundData;
    } catch (error) {
    ...
    while (true) {
        try {
            ...
            while (true) {
                ...
                console.log("Round crash point:", Number(crashPoint) / 100, "x");
                    ...
                    try {
                        ...
                        for (let i = 0; i < timeElapsed; i++) {
                            ...
                            if (currentMultiplier >= Number(crashPoint)) {
                                currentMultiplier = Number(crashPoint);
                                break;
                            }
                        }
                        ...
                        if (multiplierValue >= Number(crashPoint) / 100) {
                            const duration = (Date.now() / 1000 - roundStartTime).toFixed(1);
                            console.log(`\n\n💥 CRASHED at ${multiplierValue.toFixed(2)}x after ${duration}s`);
